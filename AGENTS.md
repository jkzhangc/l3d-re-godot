...
>
> ⚠️ **代码与文档冲突时，以代码为准，并请顺手修正文档。** 本仓库文档数量多（20+ 份）且更新不同步，已多次出现文档描述与实现不符的情况（例：README 曾声称存在"5 槽位快捷栏"，实际从未实现；本文档曾称血条素材"尚未接入"，实际 4 张早已接入）。引用任何文档结论前，建议先 `grep` 代码核实。

## 项目概述

**のび太的求生之路** — 野比大雄的生化危机 〜LEFT 3 DEAD〜（のびハザ L3D，玩法元典 Left 4 Dead）的 Godot 4.6 重制版，俯视角 2D 像素风格。
...
- **`L4D2特色机制参考.md`** — L4D2 原版机制详解（框架层参考：战役/Rush/导演/关卡/物品）
- **`L4D2缺失功能清单.md`** — 当前项目相对 L4D2 的缺失功能（⚠️ 敌人章节已被 L3D 阵容方案替代，见设计总纲 §3）
...
- **HUD/血条**：`art/Ui/` 下的血条素材是**正式游戏 HUD** 专用。其中 4 张**已接入代码**：`ＨＰバー.png`+`ＨＰメーター.png` 用于 `script/player_nameplate.gd`（联机玩家头顶 1P/2P 名牌血条），`ＨＰバーＴ.png`+`ＨＰメーターＴ.png` 用于 `scene/ui/combat_hud.tscn`（战斗 HUD 左上 HP 条）；其余 6 张（敌人配色、Boss 血条框、`GO.png` 等）仍未接入。注意 `player.gd::_draw()` 里那个手绘矩形血条是**调试可视化**（红框+绿填充，与碰撞体一起，仅在 `Global.debug_visuals` 为 true 时绘制），正式游戏不显示，二者互不相干。
...
> **现状（2026-09-02 核对）**：10 张里 **4 张已接入** —— `ＨＰバー.png` + `ＨＰメーター.png` → `script/player_nameplate.gd`（联机头顶名牌血条）；`ＨＰバーＴ.png` + `ＨＰメーターＴ.png` → `scene/ui/combat_hud.tscn`（战斗 HUD）。**仍未接入的 6 张**：`ＨＰメーターＥ.png`、`ＨＰバーＴ(heat).png`、`ＨＰバーＥ.png`、`ＨＰバーBOSS1.png`、`ＨＰバーBOSS2.png`、`GO.png`（因敌人血条、Boss 血条、低血警告、章节 GO 提示均未实现）。
> 注意 `player.gd::_draw()` 里那个手绘矩形血条是**调试可视化**（仅在 `Global.debug_visuals` 为 true 时与碰撞体一起绘制），正式游戏不显示，与本批 HUD 素材无关。
...
| 安全门/章节流程 | 🧪 基础闭环 | 安全门确认、章节总结与场景推进已接入；团灭恢复已落地；章节结算、Host 存档与完整战役验收待补齐 |
| 局域网联机 | 🧪 开发/回归中 | ENet 大厅/握手/选角、Host 权威移动战斗、敌人/子弹/掉落物同步、Client 请求式拾取、多人倒地/救援与团灭恢复、**快照插值缓冲 + 客户端本地预测 + 可靠重同步软并流、敌人死亡可靠广播（丧尸不围尸）、头顶 1P/2P 名牌与 HP 条、角色初始手枪、UPnP 自动端口映射**（2026-08-29）已接入；默认 UDP 27015（大厅可改），完整多人长流程仍未验收 |
