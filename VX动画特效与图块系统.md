...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `$tile_w_index` | 5 | 单组模式：图块组横索引（0–7），每组 64px 宽 |
| `$tile_h_index` | 1 | 单组模式：图块组纵索引（0–2），每组 160px 高 |
| `$convert_all` | true | true = 遍历全部 24 组；false = 只转上面指定的组 |
| `$write_lookup` | true | 输出 bitmask→形状→格坐标 速查表 txt |
...
| 屋顶形状行 | 47+1 | 按仿VA编辑器 `FLOOR_AUTOTILE_TABLE` 47 形状逐形状预合成（y=288..480，shape s → 格 (s%8, 9+s/8)） |
| 墙壁形状行 | 16 | 按编辑器 `WALL_AUTOTILE_TABLE` 预合成（x=256..512，y=288..352） |

> **关键约定**：内角、平行边、T 形的"墙面"部分必须从屋顶 tile 内部取（`rw_x=wx+16, rw_y=wy+48`），不能引用纯墙壁 tile，以保证墙面纹理与 roofline 色调一致。
>
> **单组输出块布局（512×480，各组文件相同）**：行 0 屋顶单/四边角，行 1-3 屋顶 3×3（y=32..96）+ 外角 16 变体（x=96..224），行 4-6 墙壁 3×3 + 内角 4，行 8 平行边/T 形（旧版，roofline 缺失，建议改用形状行），行 9-14 完整形状行。旧区域（0:0～6:4 已在街道B.tres 配 terrain 的格子）坐标与旧脚本逐条一致（114 条 blit 逐一比对 + 与旧产物像素 0 差异）。

**状态**（2026-08-30 改进版）：屋顶 3×3、外角 16 种、墙壁 3×3、内角 4 种、平行边 2 种、T 形 4 种、遍历全部 24 组、完整 47 屋顶形状 + 16 墙壁形状行、bitmask 速查表均已实现。阴影行不生成（VX Ace 墙壁阴影来自地图阴影层 z3，A4 图内无此数据，游戏侧照旧用阴影绘制逻辑）。
