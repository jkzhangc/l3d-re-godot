...
func _fire_bullet() -> void:
	...
	if effect_scene:
		## 特效偏移（2026-09-23 迁移）：按键改为武器数据（键=角色 id），与子弹偏移同构。
		var effect_offset: Vector2 = _wd.get_attack_effect_offset(character.current_character, character.facing, _wd.attack_effect_offset_override)
