...
> 设计日期：2026-08-02
> 状态：Phase 1–3 已实现；Phase 4 待实现
>
> ⚠️ **2026-09-02 方向调整**：设计方向已定为 **L3D 混合路线**（总纲见 [游戏设计方向-L3D混合](游戏设计方向-L3D混合.md)）。本文 Phase 4 的**编排框架（生成预算/间隔/紧张度阈值/事件分类/Rush 三型）继续有效**，但其中 L4D2 特感牌名需按总纲 §3 的 L3D 阵容映射替换：Hunter→Crimson Head（第一章）/Hunter 系（第二章）、Tank→Tyrant、Witch→女巫（ブレアウィッチ）、Smoker/Boomer/Spitter/Jockey/Charger 取消。涉及行：§5 特感生成表（299–313）、§7.3 Tank 事件→Tyrant 事件、§7.4 Witch 放置→女巫放置、§9/§10 对应条目与参数。第一章只需：Crimson Head 纳入普通 Rush 波次 + 单发型/耐久型两类 Rush 事件。
